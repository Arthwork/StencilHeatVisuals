...
int main(int argc, char *argv[]) {
    ...
    if (!infile || !outfile || t < 0) {
        if (rank == 0) {
            fprintf(stderr, "Usage: %s -t <num iters> -i <in> -o <out> -p <num processes> [-v <0|1|2>]\n", argv[0]);
        }
        MPI_Finalize();
        return 1;
    }
    ...
    if (p != size && rank == 0) {
        fprintf(stderr, "Warning: -p=%d but MPI world size=%d. Using MPI world size.\n", p, size);
    }
    ...
    if (rank == 0) {
        if (!read_matrix_file(infile, &global)) {
    ...
    if (rank == 0 && v >= 1) {
        printf("Input: %s\nOutput: %s\nRows: %d\nCols: %d\nIterations: %d\nProcesses: %d\n",
               infile, outfile, rows, cols, t, size);
    }
    ...
    for (int iter = 1; iter <= t; iter++) {
        ...
        /* exchange top real row with upper neighbor's bottom ghost */
        MPI_Sendrecv(&local_old[cols], cols, MPI_DOUBLE, up, 100,
                     &local_old[0], cols, MPI_DOUBLE, up, 200,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        /* exchange bottom real row with lower neighbor's top ghost */
        MPI_Sendrecv(&local_old[local_inner_rows * cols], cols, MPI_DOUBLE, down, 200,
                     &local_old[(local_inner_rows + 1) * cols], cols, MPI_DOUBLE, down, 100,
                     MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    ...
    if (rank == 0) {
        ...
        if (!write_matrix_file(outfile, &out)) {
            MPI_Abort(MPI_COMM_WORLD, 2);
        }

        double end_overall = MPI_Wtime();
        print_performance_summary(rows, size, end_overall - start_overall, end_compute - start_compute);
